...
class TestDownloadXWalk(unittest.TestCase):
    ...
    def test_download_xwalk_android_beta(self):
        ...
        for android_zip in config.ANDROID_ZIP:
            abs_zip = os.path.join(config.XWALK_BINARY_ANDROID_ROOT_DIR, 'beta', beta_version, android_zip.format(version = beta_version))
            self.assertTrue(os.path.exists(abs_zip))
    ...
    def test_download_xwalk_linux_canary(self):
        ...
        for linux_deb in config.LINUX_DEB:
            abs_linux_deb = os.path.join(config.XWALK_BINARY_LINUX_ROOT_DIRS, 'canary', canary_version, linux_deb.format(version = canary_version))
    ...
    def test_download_xwalk_windows_canary(self):
        ...
        for windows_zip in config.WINDOWS_ZIP:
            abs_zip = os.path.join(config.XWALK_BINARY_WINDOWS_DIRS.get('canary'), 
                                    canary_version, windows_zip.format(version = canary_version))
            self.assertTrue(os.path.exists(abs_zip))
    ...
    def test_download_xwalk_windows_beta(self):
        ...
        download_xwalk.download_xwalk_android('beta', beta_version)
        for windows_zip in config.WINDOWS_ZIP:
            abs_zip = os.path.join(config.XWALK_BINARY_WINDOWS_DIRS.get('beta21'), 
                                    beta_version, windows_zip.format(version = beta_version))
            self.assertTrue(os.path.exists(abs_zip))
